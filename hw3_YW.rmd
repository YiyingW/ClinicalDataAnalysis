...
**MY ANWSER:**  
feature count before filtering
```{r}
...
```
If and why the result makes sense?  
Yes. All icd9 features are filtered out and this makes sense because the icd9 code 
...
**MY ANWSER:**     
Step1: find these features' feature name in feature_matrix_filtered:  
...
Create dataframe for specific feature and outcome, plot the histogram.  
```{r}
alarms_dat <- 
...
```
Based on plotted histograms:
For features alarms and activity, they have continuous interger numerical values and their distributions are not normal. Rank-sum test should be used.
For features respiratory rate and arterial PaCO2, they have continuous interger numerical values and their distributions are approximately normal. T test can be used and rank-sum test can also be used. 
For oxydrop and snomed ct concept, Fisher's Exact test is a way to test the association between two categorical variables when there is a small cell sizes (<5). Chi-square test is used when the cell sizes are expeted to be large. Oxy_drop and snomed ct concept variables can use both of these tests. 
...
```
Both variables have p values greater than 0.05. 
...
T-test and rank-sum test for two variables give similarly small p values. 
...
For oxy_drop, two tests give similarly small p values. For snomed ct, two tests give similarly large p values. 

In summary, alarms, activity and snomed ct concept features are not associated with mortality. On the other hand, respiratory rate, arterial PaCO2 and oxy_drop features are associated with mortality at a statistically significant level. 
...
signif_chart_value <-
  ...
  mutate(name=substr(name, 1, nchar(name)-8)) %>% # cut the proper name
...
The point estimate changes as more features are added because the newly added features are correlated with that parameter's corresponding variable and correlated with the response variable as well.   
...
If adding X2 doesn't change X1 coefficient, then X2 is not correlated with X1 and it is not correlated with Y.
...
**MY ANWSER:**    
I think these confidence intervals are legitimate for all of these models. I came to this conclusion by comparing the size of the confidence intervals which indicates the size of the standard errors of coefficient for oxy_drop. Model1's oxy_drop coefficent confidence interval's size is 0.3681231. Model2's oxy_drop coefficent confidence interval's size is 0.3686322. Model1's oxy_drop coefficent confidence interval's size is 0.4166014. As adding more features, the coefficent's confidence interval is getting bigger, implying the standard errors for coefficent is becoming bigger. This makes sense because the more correlated the features are with each other, the bigger the standard errors become for their coefficents, and the less likely it is that a coefficient will be statistically significant. This is known as the problem of multicollinearity. Model3 compared with model1 and model2 has more correlated independent variables, therefore, it is more difficult to determine how much variation in outcome that oxy_drop is responsible for. As a result, the standard errors for oxy_drop's coefficent become larger.   
...
patients_survival <-
  ...
  mutate(survival_time=ceiling(as.numeric(difftime(as.POSIXct(censor_time), 
...
```
Note: patients_survival has four columns. survival_time, oxy_drop, censor(0=died, 1=censored), event (0=censored, 1=died). 
...
Use functions in these packages to generate Kaplan-Meier curves for the survival data you created above. How do they compare to the curves generated with your code? What features do these packages provide that your code does not?  
```{r}
library(survival)
...
```{r}
uni.model <- coxph(surv_obj~feature_matrix_filtered[, 'oxy_drop'])
summary(uni.model)
```

The value of coefficient is 0.06528 and thus the odds ratio is exp(coef) = 1.067. The confidence interval for exp(coef) (odds ratio) is 0.9063 to 1.257.  
...
feature_matrix_filteredoxy_drop            -2.417403e-01 7.852601e-01 1.098356e-01 -2.2009289356 2.774106e-02
...
ggplot(calibration_plot) +
  ...
  annotate("text", x=0.2, y=0.9, label=as.character(r_squared), size=4)
...
The AUC of this model on the test set is 0.8097. It is close but a little bit higher than that was estimated by cross-validation. 
...
**MY ANWSER:**    
This method is not a good method. I would expect the cross-validation AUC to be on average larger than the test set AUC. Because this supervised feature selection (using correlation with class labels) was performed on the whole training set and this makes the test data in each fold of cross validation has been seen by the model when choosing the features which may result in overfitting.  
If I needed to reduce the feature space in this way, I would include using class labels for feature selection as part of my cross validation procedure. More specifically, I would define the folds, remove a fold, filter features on the other folds, select set of features, produce the model, then use it on the held out fold. 
...
**MY ANWSER:**    
No, what described above in 3.3.3 is not the same as we did earlier by removing the near-zero-variance features. Because in removing the near-zero-variance we removed the features based on its own distribution. We didn't use any information from class labels or their associations, therefore we didn't peek at the test dataset like we did in 3.3.3. 
...
Yes. The effect of age is nearly linear in the range of normal human lifetime (20,000 days to 30,000 days). There are apparently outliers in age_in_days variable.   
...
**MY ANWSER:**   
It is strange about the range of the age variable in the partial dependence plot is that there are some people with unrealistic high age_in_days value (over 200 years old). 
```{r}
ggplot(PredictiveMatrix)+
  geom_histogram(aes(x=age_in_days), binwidth=1000)
```

This means the data has outliers. Maybe there were some mistakes when data was recorded. I don't think this adversely affects the test set performance of the GBT model because this model is not sensitive to outliers. It would probability still split the data at the same position if there were no such outliers. There will make age feature lose predictive effect on the linear model. The coefficient for age_in_days in the linear model is -1.244906e-05. This is very small so age doesn't have much predictive power in linear model because of the outliers. 
